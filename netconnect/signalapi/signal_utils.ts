...
export class SignalProcessor {
  ...
  latestOfType(signals: Signal[], type: string): Signal | null {
    ...
    return filtered.reduce((a, b) => (b.timestamp > a.timestamp ? b : a))
  }
  ...
  summarize(signal: Signal): string {
    ...
    return `[${time}] ${signal.type.toUpperCase()}${severity}${source}: ${JSON.stringify(signal.payload)}`
  }

  /**
   * Batch summarize multiple signals into strings.
   */
